...
const StoreFront = () => {
  ...
  const {
    ...
    queryFn: () => {
      ...
      return getRequest(`/view-athlete/${slug}`).catch(() => {
        // If slug endpoint fails, try ID endpoint as fallback
        return getRequest(`/view-athlete/${slug}`);
      });
    },
  ...
  // Fetch athlete's products if slug is provided
  const {
    data: athleteProducts,
  ...
  return (
    <>
      ...
      >
        ...
        {slug && athlete && (athlete?.profile_picture_url || athlete?.profile_picture) &&(
