...
const stripePromise = loadStripe(
  "pk_live_51REvJXJie2PUnhtx7uy53YLEKNxXqSqb6Fa8BdN2KGsoK7CzmfURna8ekI7q72P6dg5OocNOf243wpTsODrTqtUQ00Fds6auka"
);
...
const Subscription = () => {
  ...
  const subscriptionModal = useModalHistory(
    "subscriptionModal",
    isShow !== false,
    () => setIsShow(false)
  );
              ...
              {data?.map(
                (item) =>
                  item?.is_active == 1 && (
                    <div
                      key={item?.id}
                      className="relative px-4 sm:px-6 lg:px-8 pt-12 sm:pt-14 lg:pt-16 pb-6 sm:pb-8 lg:pb-10 card-gradient !border-[1.5px] rounded-3xl w-full sm:w-[25.375rem]"
                    >
                      <div
                        className={`absolute top-3 sm:top-4 lg:top-5 right-4 sm:right-0 sm:-translate-x-1/2 text-white text-sm sm:text-base capitalize font-semibold px-3 sm:px-4 py-1 rounded-full shadow-md ${getColor(
                          item?.type
                        )}`}
                      >
                        {item?.type}
                      ...
                      <div className="">
                        ...
                        <img
                          src="/line2.svg"
                          alt="line"
                          className="w-full mb-6 sm:mb-8 lg:mb-11"
                        />
                      </div>
                      <div className="flex gap-3 sm:gap-4 lg:gap-5 flex-col">
                        {item?.points
                          ...
                          ?.map((point, index) => (
                            <div
                              key={index}
                              className="flex gap-3 sm:gap-4 lg:gap-5 items-center"
                            >
                              <div className="">
                                <svg
                                  width={"1.2rem"}
